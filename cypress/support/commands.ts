...
Cypress.Commands.add(
	...
	(
		embeddingOptions,
		endpointUrl = "https://endpoint-trial.cognigy.ai/5e51fcdc2c10fe4c5267c8a798a7134086f60b62998062af620ed73b096e25bd",
	) => {
		return cy
			.window()
			.then(window => {
...
Cypress.Commands.add(
	"withMessageFixture",
	(filename: string, callbackFunc: () => void, cleanUpFunc: () => void) => {
		cy.fixture(`messages/${filename}.json`).then(json => {
			if (Array.isArray(json)) {
				json.forEach((message, index) => {
					cy.log(`Fixture ${filename} #${index + 1} of ${json.length}`);
					cy.receiveMessage(message?.text ?? null, message?.data ?? {}, "bot").then(
						callbackFunc,
					);
					if (typeof cleanUpFunc === "function" && index < json.length - 1)
						cy.then(cleanUpFunc);
				});
			} else {
...
Cypress.Commands.add("renderMessage", (text: string, data: any, source: string, config: any) => {
	cy.window().then(window => {
		...
		// @ts-ignore
		window.MessageRenderer.renderMessage({ text, data, source }, root, config);
	});
		...
		() => {
			return cy.getHistory().then(history => {
				if (typeof messageOrMatcher === "function") {
					...
					const matchData = messageOrMatcher.data
						? JSON.stringify(messageOrMatcher.data)
						: undefined;

					return !!history.find(message => {
						if (matchData && JSON.stringify(message.data) !== matchData) return false;

						if (text && message.text !== text) return false;
