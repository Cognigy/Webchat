describe("Message Templates with null Buttons", () => {
	...
	it("renders a gallery with null buttons", () => {
		cy.fixture("messages/gallery-with-null-buttons.json").then(({ text, data, source }) => {
	...
	it("renders a list with null buttons", () => {
		cy.fixture("messages/list-with-null-buttons.json").then(({ text, data, source }) => {
			cy.receiveMessage(text, data, source);
		});
	...
	it("renders a text with buttons with null buttons", () => {
		cy.fixture("messages/buttons-with-null-buttons.json").then(({ text, data, source }) => {
			cy.receiveMessage(text, data, source);
		});
	...
	it.skip("renders text+quick replies with null quick replies", () => {
		cy.fixture("messages/quick-replies-with-null-quick-replies.json").then(
			({ text, data, source }) => {
				cy.receiveMessage(text, data, source);
			},
		);
