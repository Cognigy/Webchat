...
export const createConfigMiddleware =
	(
		url: string,
		overrideWebchatSettings?: Partial<IWebchatSettings>,
	...
	(action: LoadConfigAction) => {
		switch (action.type) {
			case "LOAD_CONFIG": {
				...
				(async () => {
					const endpointConfig = await fetchWebchatConfig(
						url,
						(store.getState().config.settings.embeddingConfiguration?.connectivity
							?.enabled &&
							store.getState().config.settings.embeddingConfiguration?.connectivity
								?.timeout) ||
							4000,
					...
					if (endpointConfig) {
						...
						const ttsActive =
							...
							(!considerTTSOptions || settings.chatOptions.activateTTSToggle);

						store.dispatch(setTTSActive(ttsActive));
					} else if (overrideWebchatSettings) {
						store.dispatch(
							setConfig({
								settings: overrideWebchatSettings,
								isTimedOut: true,
								isConfigLoaded: true,
							}),
						);
