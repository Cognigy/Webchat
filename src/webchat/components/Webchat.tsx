...
export class Webchat extends React.PureComponent<WebchatProps> {
	...
	open = async () => {
		...
		const timeout =
			(this.store.getState().config.settings.embeddingConfiguration?.connectivity?.enabled &&
				this.store.getState().config.settings.embeddingConfiguration?.connectivity
					?.timeout) ||
			1000;
