...
export const isOutOfBusinessHours = (businessHours: IWebchatSettings["businessHours"]): boolean => {
	if (businessHours && businessHours.enabled) {
		...
		const businessHoursToday = businessHours.times.filter(item => {
			return item.weekDay === now.weekDay;
		});
		for (let i = 0; i < businessHoursToday.length; i++) {
			...
			if (parseInt(hourStart) < now.hour && now.hour < parseInt(hourEnd)) {
				...
				parseInt(hourStart) === now.hour &&
				now.minute >= parseInt(minuteStart) &&
				now.hour < parseInt(hourEnd)
			) {
				outOfBusinessHours = false;
			} else if (
				parseInt(hourStart) === now.hour &&
...
export const isHiddenOutOfBusinessHours = (
	businessHours: IWebchatSettings["businessHours"],
): boolean => {
...
export const isDisabledOutOfBusinessHours = (
	businessHours: IWebchatSettings["businessHours"],
): boolean => {
...
export const isInformingOutOfBusinessHours = (
	businessHours: IWebchatSettings["businessHours"],
): boolean => {
