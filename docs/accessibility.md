...
When a screen appears whose focus target doesn't convey the screen change, announce the screen's context through the always-mounted status live region instead of moving focus to a heading/region — use `<ScreenAnnouncer active label>` (`src/webchat-ui/components/presentational/ScreenAnnouncer.tsx`), which defers the announcement past the slide transition and the screen's own focus move so it isn't cancelled by the focus utterance. Used for the home screen title (focus lands on the close button, CGY-3276; announced on every appearance).

### Pattern: the AI-agent notice (chat-screen intro text, CGY-3519)

Static intro text at the top of the chat log ("You're now chatting with an AI Agent." / `AIAgentNoticeText`) can't announce itself — it renders together with the screen, so no live region registers it as a change, focus lands in the message input _past_ it in reading order, and `aria-describedby` on the input would be re-read on every re-focus (the input re-focuses after each response). It is announced as an **intro coordinated with the chat log's message live region** (`introText` on `<ScreenReaderLiveRegion>`), **not** through the status region: while the intro is pending, message announcements **hold** and resume right after it, so the live-region mutations always reach the screen reader in intro-first order (the PostNL acceptance criterion) — uncoordinated regions interleave in raw mutation order. The intro renders into its **own** region (`#webchatAIAgentNoticeLiveRegion`, a sibling of the message region) rather than the message region itself: that region shows one child at a time, so the next message announcement would replace the intro's node — and NVDA silently drops a queued polite announcement whose node leaves the DOM before it is processed (reproduced on session switches, where the overlay utterances keep NVDA's queue busy). The intro itself is deferred 600ms past the screen's focus utterances.

Announced once per **brand-new** conversation: starting a new session announces, while navigating away and back within the same session or reopening a previous conversation stays silent. New-ness is evaluated against a pre-switch snapshot of `prevConversations` (`computeNoticeSession` in `src/webchat-ui/utils/ai-agent-notice.ts`) because a genuinely new session gets upserted into that map by auto-inject in the same update batch; the page load's first conversation keeps its announcement key across the connect that assigns its session id, so it is neither double-announced nor skipped. Announced keys live on the `WebchatUI` instance (`announcedNoticeKeys`) because the live region remounts on every screen navigation. The intro is held while the disconnect overlay is open (session switches reconnect the socket and open it — its dialog and focus utterances would cancel the notice) and becomes pending again when it closes.
