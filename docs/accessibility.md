...
When a screen appears whose focus target doesn't convey the screen change, announce the screen's context through the always-mounted status live region instead of moving focus to a heading/region — use `<ScreenAnnouncer active label>` (`src/webchat-ui/components/presentational/ScreenAnnouncer.tsx`), which defers the announcement past the slide transition and the screen's own focus move so it isn't cancelled by the focus utterance. Two instances exist: the home screen title (focus lands on the close button, CGY-3276; announced on every appearance) and the chat screen's AI-agent notice (focus lands in the message input, past the visible notice in reading order, so "You're now chatting with an AI Agent." would otherwise never be read; CGY-3519). The notice instance passes `once` keyed by session — it announces once per **brand-new** conversation: starting a new session (home screen, or "Start new conversation" under previous conversations) announces, while navigating away and back within the same session or reopening a previous conversation stays silent. New-ness is evaluated against a pre-switch snapshot of `prevConversations` (`evaluateNoticeSession` in `WebchatUI.tsx`) because a genuinely new session gets upserted into that map by auto-inject in the same update batch; the page load's first conversation keeps its announcement key across the connect that assigns its session id, so it is neither double-announced nor skipped. Closing and reopening the webchat announces again (matching the status region's mount lifetime), except for a conversation restored into `prevConversations`. Static intro text at the top of a new screen can't announce itself — it renders together with the screen, so no live region registers it as a change, and `aria-describedby` on the auto-focused input would be re-read on every re-focus (the input re-focuses after each response).
