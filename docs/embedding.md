...
| enableIconAnimationPauseButton | boolean | `false`                                 | Opt-in. Renders a pause/resume button on the launcher's edge while `iconAnimation` is set, so users can stop the animation without opening the chat (WCAG 2.2.2). See [docs/accessibility.md](./accessibility.md#pattern-pausable-launcher-animation-iconanimationpausebutton-cgy-39786) |
...
With either `enableIndicator` or `enableBadge` on, the unread count is also announced to screen readers through a `role="status"` live region whenever it changes (see [docs/accessibility.md](./accessibility.md)); the announcement text comes from `customTranslations.ariaLabels.unreadMessageSingularText` / `unreadMessagePluralText`.
...
| emptyListText                  | string  | `"No previous conversations"` | Text shown in the list when there are no previous conversations. After "Delete all conversations" is confirmed, focus moves to this text so screen readers read the outcome         |
...
| scrollLockAllowSelectors | string[] | `[]` | CSS selectors for host-page elements that must stay scrollable while the webchat holds the mobile scroll lock (open fullscreen on a mobile-sized viewport). The lock stays active for the rest of the page, and only the matched elements (such as your own custom overlay or banner) remain scrollable. Common consent managers (OneTrust, Cookiebot, Usercentrics, Didomi) are always allowed, so a typical cookie-consent banner needs no configuration here. To turn the lock off entirely instead, see `disableMobileScrollLock`. |
| disableMobileScrollLock | boolean | `false` | When the webchat is open on a mobile-sized viewport it locks scrolling of the underlying host page. Set this to true to opt out of that lock completely, so the host page scrolls freely while the webchat is open. This is a broad opt-out. If you only need a specific host-page overlay to stay scrollable while the rest of the page stays locked, use `scrollLockAllowSelectors` instead. |
...
| customAllowedHtmlTags | string[] | `undefined` | Custom list of allowed HTML tags that completely overrides the default allowed tags. When provided, only these tags will be allowed in HTML content sanitization. Set to `[]` to strip all HTML tags. Example: `["p", "br", "strong", "em"]` allows only paragraphs, line breaks, bold and italic text. If the value is not an array, it is ignored and the default allow-list is used. Non-string entries in the array are silently dropped. **Security note:** the following 16 tags are always stripped from this list before it is applied — they cannot be re-enabled by tenant configuration: `script`, `iframe`, `object`, `embed`, `applet`, `frame`, `frameset`, `noframes`, `meta`, `base`, `link`, `style`, `form`, `body`, `html`, `head`. |
...
| unreadMessages            | string   | "Unread messages"                          | Deprecated, no longer read. The unread badge is decorative; its count is part of the toggle button's name and is announced live via `unreadMessageSingularText` / `unreadMessagePluralText`.                                                                                                                                                                   |
| unreadMessageSingularText | string   | "One unread message in chat"               | Label for a single unread message: part of the chat toggle button's name and announced live (`role="status"`) when the count changes.                                                                                                                                                                                                                          |
| unreadMessagePluralText   | string   | "unread messages in chat"                  | Label for multiple unread messages, prefixed with the count: part of the chat toggle button's name and announced live (`role="status"`) when the count changes.                                                                                                                                                                                                |
...
| xAppOverlay               | string   | "Embedded app"                             | Name of the xApp overlay dialog (and the title of its frame) when the xApp has no screen title.                                                                                                                                                                                                                                                                |
...
| datePickerGridLabel       | string   | "Calendar"                                 | This text is used as the ARIA label for the date-picker calendar grid, announced to screen-reader users.                                                                                                                                                                                                                                                       |
| datePickerGridDescription | string   | "Use arrow keys to navigate through dates" | This text is used as the ARIA description for the date-picker calendar grid, instructing screen-reader users how to navigate between dates.                                                                                                                                                                                                                    |
| datePickerRangeStart      | string   | "start of range"                           | Appended to a day's ARIA label in range-selection date-pickers to mark the first selected day, announced to screen-reader users.                                                                                                                                                                                                                               |
...
interface IWebchatSettings {
	...
	customTranslations?: {
		...
		cancel?: string;
		// Status message announced after a conversation is deleted from the chat options screen
		...
		ariaLabels?: {
			...
			// Webchat toggle (launcher) animation control; the name switches with the icon
